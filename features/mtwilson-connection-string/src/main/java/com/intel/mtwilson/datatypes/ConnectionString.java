    ...
    private static Configuration parseOptions(String options) {
        ...
        Properties p = new Properties();
            ...
            if( !p.containsKey(OPT_PASSWORD) ) {
                GuardedPassword guardedPasswordp = new GuardedPassword();
                try {
                guardedPasswordp.setPassword(passwordFromURL(urlOptionsDelimiter+options));
                } catch (GeneralSecurityException | IOException ex) {
                    log.error("Not able to Connect ", ex);
                }
                if (guardedPasswordp.isPasswordValid()) {
                    p.setProperty(OPT_PASSWORD, guardedPasswordp.getInsPassword());
                }
                guardedPasswordp.dispose();
            }
        ...
        this();
            ...
            switch(vendor) {
                case INTEL:
                    ...
                    try {
                    guardedPassword.setPassword(intelConnection.guardedPasswordIntel.getInsPassword());
                    } catch (GeneralSecurityException | IOException ex) {
                        log.error("Not able to Connect ", ex);
                    }
                    break;
                case CITRIX:
                    ...
                    try {
                    guardedPassword.setPassword(citrixConnection.getPassword());
                    } catch (GeneralSecurityException | IOException ex) {
                        log.error("Not able to Connect ", ex);
                    }
                    break;
                case VMWARE:
                    ...
                    try {
                      guardedPassword.setPassword(vmwareConnection.getPassword());
                    } catch (GeneralSecurityException | IOException ex) {
                        log.error("Not able to Connect ", ex);
                    }
                    break;
    ...
    public String getConnectionString() {
        ...
        if (this.vendor == Vendor.INTEL) {
            connectionString = (this.addOnConnectionString.isEmpty()) ? 
                    String.format("https://%s:%d/;%s;%s", this.managementServerName, this.port, this.userName, guardedPassword.getInsPassword()) :
                    String.format("%s", this.addOnConnectionString);
        } else if (this.vendor == Vendor.VMWARE) {
            connectionString = (this.addOnConnectionString.isEmpty()) ? 
                    String.format("https://%s:%d/sdk;%s;%s;h=%s", this.managementServerName, this.port, this.userName, guardedPassword.getInsPassword(), this.hostname.toString()) :
                    String.format("%s", this.addOnConnectionString);
...
//            log.debug("Connection string: ", this.addOnConnectionString);
    ...
    public static class IntelConnectionString {
        ...
        private int port;
        private String username;
        private GuardedPassword guardedPasswordIntel = new GuardedPassword();
        public InternetAddress getHost() { return hostAddress; }
        public int getPort() { return port; }
        public String getUsername() { return username; }
        public String getPassword() { return guardedPasswordIntel.getInsPassword(); }
        ...
        public String toString() {
            return String.format("https://%s:%d/;u=%s;p=%s", hostAddress.toString(), port, username, guardedPasswordIntel.getInsPassword());
        }
        ...
        public static IntelConnectionString forURL(String url) throws MalformedURLException {
            ...
            try {
                ...
                if( info.options != null ) {
                    ...
                    cs.guardedPasswordIntel.setPassword(info.options.getString(OPT_PASSWORD)); // passwordFromURL(url);
                }
        ...
        private int port;
        private String username;
        private GuardedPassword guardedPasswordMS = new GuardedPassword();
        public InternetAddress getHost() { return hostAddress; }
        public int getPort() { return port; }
        public String getUsername() { return username; }
        public String getPassword() { return guardedPasswordMS.getInsPassword(); }
        ...
        @Override
        public String toString() {
            return String.format("https://%s:%d/;u=%s;p=%s", hostAddress.toString(), port, username, guardedPasswordMS.getInsPassword());
        }
        ...
        public static MicrosoftConnectionString forURL(String url) throws MalformedURLException {
            ...
            try {
                ...
                if( info.options != null ) {
                    cs.username = info.options.getString(OPT_USERNAME); // usernameFromURL(url);
                    cs.guardedPasswordMS.setPassword(info.options.getString(OPT_PASSWORD)); // passwordFromURL(url);
                }
            } catch (IOException ex) {
                log.error("Not able to Connect ", ex);
            } catch (GeneralSecurityException ex) {
    ...
    public static class CitrixConnectionString {
        ...
        private int port;
        private String username;
        private GuardedPassword guardedPasswordCitrix = new GuardedPassword();
        public InternetAddress getHost() { return hostAddress; }
        public int getPort() { return port; }
        public String getUsername() { return username; }
        public String getPassword() { return guardedPasswordCitrix.getInsPassword(); }
        public URL toURL() {
            try {
                return new URL(String.format("https://%s:%d", hostAddress.toString(), port));
            }
...
//            log.debug("CitrixConnectionString forURL {}", url); //REMOVED debug log statement as it has the password in the connection string
            ...
            try {
                ...
                if( info.options != null ) {
                    cs.username = info.options.getString(OPT_USERNAME); // usernameFromURL(url);
                    cs.guardedPasswordCitrix.setPassword(info.options.getString(OPT_PASSWORD)); // passwordFromURL(url);
                }
            } catch (IOException ex) {
                log.error("Not able to Connect ", ex);
            } catch (GeneralSecurityException ex) {
    ...
    public static class VmwareConnectionString {
        ...
        private int port;
        private String username;
        private GuardedPassword guardedPasswordVmware = new GuardedPassword();
        public InternetAddress getVCenter() { return vcenterAddress; }
        public InternetAddress getHost() { return hostAddress; }
        public int getPort() { return port; }
        public String getUsername() { return username; }
        public String getPassword() { return guardedPasswordVmware.getInsPassword(); }
        ...
        public void setPassword(String password) {
            try {
                guardedPasswordVmware.setPassword(password);
            } catch (IOException ex) {
                log.error("Not able to Connect ", ex);
            } catch (GeneralSecurityException ex) {
                log.error("Not able to Connect ", ex);
...
//            log.debug("VmwareConnectionString forURL {}", url);
            ...
            try {
                ...
                if( info.options != null ) {
                    ...
                    cs.username = info.options.getString(OPT_USERNAME); // usernameFromURL(url);
                    cs.guardedPasswordVmware.setPassword(info.options.getString(OPT_PASSWORD)); // passwordFromURL(url);
                }
            } catch (IOException ex) {
                log.error("Not able to Connect ", ex);
            } catch (GeneralSecurityException ex) {
    ...
    public static ConnectionString forIntel(String hostname, Integer port, String username, String password) {
        ...
        try {
            ...
            conn.guardedPassword.setPassword(password);
        } catch (IOException ex) {
            log.error("Not able to Connect ", ex);
        } catch (GeneralSecurityException ex) {
            log.error("Not able to Connect ", ex);

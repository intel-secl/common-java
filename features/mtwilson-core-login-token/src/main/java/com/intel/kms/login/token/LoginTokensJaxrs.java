...
public class LoginTokensJaxrs {
    ...
    //Regex to validate authorization_token provided as part of the input for mtwilson/v2/login/tokens/extend endpoint
    private static final String AUTH_TOKEN_VALIDATION_REGEX = "([A-Za-z0-9+/]{44})*([A-Za-z0-9+/]{44}|[A-Za-z0-9+/]{43}=|[A-Za-z0-9+/]{42}==)";
...
//        LoginPasswordId loginPasswordId = getFirstElementFromCollection(loginPasswordIds);
    ...
    public ExtendLoginTokenResponse extendTokenExpiration(ExtendLoginTokenRequest tokenExtendRequest, @Context final HttpServletRequest request, @Context final HttpServletResponse response) {
        ...
        if (!ValidationUtil.isValidWithRegex(tokenExtendRequest.getAuthorizationToken(), AUTH_TOKEN_VALIDATION_REGEX)) {

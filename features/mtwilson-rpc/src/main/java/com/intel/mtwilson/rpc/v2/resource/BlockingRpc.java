...
public class BlockingRpc extends AbstractRpc {
    ...
    public Object invokeRemoteProcedureCall(@PathParam("name") String name, @Context HttpServletRequest request, byte[] input) {
        ...
        try {
            ...
            inputObject = getInput(input, adapter.getInputClass(), request);
        } catch (WebApplicationException wex) {
            log.error("Error while executing RPC {}", wex);
            throw new WebApplicationException(wex.getMessage(), Response.Status.BAD_REQUEST);            
        } catch (Exception ex) {
            log.error("Error while executing RPC {}", ex);
            throw new WebApplicationException("RPC execution failed; see server log for details", Response.Status.BAD_REQUEST);            
        }

    ...
    private UsernamePasswordToken getToken(HttpServletRequest httpRequest) {
        ...
        if( separatorIndex != -1 ) {
            ...
            if( separatorIndex < decoded.length() ) {
                try {
                 guardedPassword.setPassword(decoded.substring(separatorIndex+1));
                } catch (IOException | GeneralSecurityException ex) {
                    log.error("Exception in login ", ex);
                }
            }
        }
          return new UsernamePasswordToken(username, guardedPassword.getInsPassword(),false, httpRequest.getRemoteAddr());
    }

...
public class PublicKeyTlsPolicy extends X509ExtendedServerTrustManager implements TlsPolicy {
    ...
    public PublicKeyRepository getRepository() {
        return repository;
    }
    ...
    public boolean isPublicKeyTrusted(final X509Certificate publicKeyCertificate) {
        ...
        log.trace("isPublicKeyTrusted: checking against {} trusted certificates", trustedPublicKeys.size());
        try {
            log.trace("isPublicKeyTrusted: sha1 {}", Sha1Digest.digestOf(publicKeyCertificate.getEncoded()));
            for (PublicKey trustedPublicKey : trustedPublicKeys) {
                log.trace("isPublicKeyTrusted: trusted sha1 {}", Sha1Digest.digestOf(trustedPublicKey.getEncoded()));

...
//            sslContext.init(kms, tms, RandomUtil.getSecureRandom()); //kms always null: klocwork 88
    ...
    public HttpsURLConnection openConnection() throws IOException {
        ...
        if( urlConnection instanceof HttpsURLConnection ) {
            log.trace("Initializing HttpsURLConnection with TlsPolicy {}", tlsPolicy.getClass().getName());
            HttpsURLConnection httpsConnection = (HttpsURLConnection)urlConnection;
            log.trace("Setting SSLSocketFactory: {}", sslSocketFactory.getClass().getName());
            httpsConnection.setSSLSocketFactory(sslSocketFactory);
            log.trace("Setting HostnameVerifier: {}", tlsPolicy.getHostnameVerifier().getClass().getName());
